...
class Kira_Lite_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			'enable_post_breadcrumbs' => get_theme_mod( 'kira_lite_enable_post_breadcrumbs', 1 ),
			'home_prefix' => get_theme_mod('kira_lite_blog_breadcrumb_menu_prefix', __('You Are Here', 'kira-lite') ),
			'separator' => get_theme_mod('kira_lite_blog_breadcrumb_menu_separator', 'rarr'),
			'show_post_type_archive' => '1',
			'show_terms' => get_theme_mod('kira_lite_blog_breadcrumb_menu_post_category', 1),
			'home_label' => esc_html__( 'Home', 'kira-lite' ),
			'tag_archive_prefix' => esc_html__( 'Tag:', 'kira-lite' ),
			'search_prefix' => esc_html__( 'Search:', 'kira-lite' ),
			'error_prefix' => esc_html__( '404 - Page not Found', 'kira-lite' ),
		);

		// Setup a filter for changeable variables and meger it with the defaults
		$args = apply_filters( 'kira_lite_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		if( $this->enable_post_breadcrumbs == 1 ) {
			$this->html_markup = sprintf( '<nav class="breadcrumb hidden-xs"><div class="out-left white"></div><ul class="clearfix">%s</ul></nav>', $this->html_markup );
		}
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<li class="rl-breadcrumb-prefix">%s:</li>', esc_html( $this->home_prefix ) );
			}
		}
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
		...
		if ( $separator ) {
			$separator_markup = sprintf( '<li class="rl-breadcrumb-sep">%s</li>', $this->separator );
		}

		return sprintf( '<li %s>%s</li>%s', $microdata_itemscope, $breadcrumb_content, $separator_markup );
	}
}